...
export default function CostSheet() {
  ...
  const { data: prodsData } = useQuery({ queryKey:['products'], queryFn: () => fetchProducts({ limit: 500 }) })
  const { data: sizesRes } = useSizesQuery({ is_active: 'true', limit: 100 })
  
  const fgProducts = prodsData?.data?.filter(p => p.product_type === 'FINISHED') || []
  const activeSizes = sizesRes?.data || []

  // Run query only if fgSku
    ...
    <div className="space-y-6">
      <div className="card p-4 flex flex-wrap items-end gap-4 justify-between bg-white border border-gray-200">
        <div className="flex items-center gap-4">
          <div>
            ...
            <select className="input-field py-1.5 min-w-[250px]" value={fgSku} onChange={e => setFgSku(e.target.value)}>
               <option value="">— Select Product —</option>
               {fgProducts.map(p => <option key={p.sku_code} value={p.sku_code}>{p.sku_code} - {p.description}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs font-semibold text-gray-500 mb-1 block">Size Override</label>
            <select className="input-field py-1.5 min-w-[150px]" value={sizeCode} onChange={e => setSizeCode(e.target.value)}>
               <option value="">All Sizes</option>
               {activeSizes.map(s => <option key={s.size_code} value={s.size_code}>{s.size_label}</option>)}
            </select>
          </div>
        </div>
        <button onClick={handleExport} className="btn-secondary text-sm">Export All Products (Excel)</button>
      </div>
      ...
      {!isLoading && sheet && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="card p-5 bg-gradient-to-br from-blue-900 to-indigo-900 text-white flex flex-col justify-center shadow-lg border-empty">
              <p className="text-xs font-bold text-blue-300 uppercase tracking-widest">Selected Product</p>
              <p className="text-xl font-black mt-1 leading-tight">{sheet.product.sku}</p>
              <p className="text-sm text-blue-100">{sheet.product.description}</p>
            </div>
            <div className="card p-5 bg-white border border-gray-100 flex flex-col justify-center">
              ...
              <p className="text-3xl font-black mt-2 tabular-nums text-gray-800">{formatCurrency(sheet.costSheet.averageCost)}</p>
            </div>
            <div className="card p-4 bg-white border border-gray-100 grid grid-cols-2 gap-2">
               <div className="flex flex-col justify-center">
                  ...
                  <p className="text-lg font-black tabular-nums text-green-600 mt-1">{formatCurrency(sheet.costSheet.lowestCost?.value || 0)} <span className="text-xs font-semibold text-gray-400">({sheet.costSheet.lowestCost?.size})</span></p>
               </div>
               <div className="flex flex-col justify-center">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Highest Cost Bound</p>
                  <p className="text-lg font-black tabular-nums text-red-600 mt-1">{formatCurrency(sheet.costSheet.highestCost?.value || 0)} <span className="text-xs font-semibold text-gray-400">({sheet.costSheet.highestCost?.size})</span></p>
               </div>
            </div>
          </div>

          <div className="grid grid-cols-1 gap-6">
             {Object.entries(sheet.costSheet.bySize).map(([size, dt]) => (
                <div key={size} className="card p-0 overflow-hidden border border-gray-200">
                  <div className="px-5 py-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                     ...
                     <span className="font-mono font-bold text-lg text-blue-700">{formatCurrency(dt.materialCost)}</span>
                  </div>
                       ...
                       <tbody>
                         {dt.materialBreakdown.map((m, i) => (
                           <tr key={i} className="border-t border-gray-100 hover:bg-gray-50/50 transition-colors">
                             ...
                             <td className="px-5 py-2 text-right tabular-nums font-bold text-blue-800 bg-blue-50/20 border-x border-blue-50">{m.qty.toFixed(4)}</td>
                             <td className="px-5 py-2 text-right tabular-nums text-gray-500">{m.rate.toFixed(2)}</td>
                             <td className="px-5 py-2 text-right tabular-nums font-bold text-gray-800">{m.value.toFixed(2)}</td>
                           </tr>
                         ))}
                       </tbody>
                     </table>
      ...
      {!isLoading && !fgSku && (
         <div className="py-20 text-center border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
            <p className="text-gray-400 font-medium">Select a Finished Product to view its multi-size Cost Sheet.</p>
         </div>
      )}
    </div>
  )
