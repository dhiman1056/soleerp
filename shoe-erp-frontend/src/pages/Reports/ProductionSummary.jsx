...
export default function ProductionSummary() {
  ...
  // queryFn already returns the unwrapped data object (or safe fallback)
  const summary   = data?.summary  ?? {};
  const byProduct = Array.isArray(data?.byProduct)  ? data.byProduct  : [];
  const byWoType  = Array.isArray(data?.byWoType)   ? data.byWoType   : [];
  ...
  const pieData = byWoType.map(v => ({
    name:  v?.wo_type ?? 'Unknown',
    value: Number(v?.orders ?? 0),
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="card p-0 overflow-hidden border border-gray-200">
        <h3 className="px-5 py-4 text-sm font-bold text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
          Production by Product (Top Metrics)
        </h3>
