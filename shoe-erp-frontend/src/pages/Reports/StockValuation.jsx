...
export default function StockValuation() {
  ...
  const pieData = [
    { name: 'Raw Material', value: rep.summary.byCategory.RAW_MATERIAL.value },
    { name: 'Semi-Finished', value: rep.summary.byCategory.SEMI_FINISHED.value },
    { name: 'Finished Product', value: rep.summary.byCategory.FINISHED.value },
  ].filter(d => d.value > 0)
    ...
    <div className="space-y-6">
      <div className="card p-4 flex flex-wrap items-end gap-4 justify-between bg-white border border-gray-200">
        <div>
          ...
          <select className="input-field py-1.5 min-w-[200px]" value={category} onChange={e => setCategory(e.target.value)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-4">
           <div className="card p-6 bg-gradient-to-r from-blue-900 to-indigo-900 text-white flex flex-col justify-center shadow-lg border-empty flex-1">
              <p className="text-xs font-bold text-blue-300 uppercase tracking-widest">Total Stock Valuation</p>
              <p className="text-4xl font-black mt-2 tabular-nums">{formatCurrency(rep.summary.totalValue)}</p>
           </div>
           <div className="grid grid-cols-2 gap-4">
              <div className="card p-5 bg-white border border-gray-100 flex flex-col justify-center">
                 ...
                 <p className="text-2xl font-black mt-1 tabular-nums text-gray-800">{rep.summary.totalSkus}</p>
              </div>
              <div className="card p-5 bg-white border border-gray-100 flex flex-col justify-center relative overflow-hidden">
                 ...
                 <p className="text-2xl font-black mt-1 tabular-nums text-amber-600">{rep.items.filter(i => i.isLowStock).length}</p>
      ...
      <div className="card p-0 overflow-hidden border border-gray-200">
        <h3 className="px-5 py-4 text-sm font-bold text-gray-700 uppercase bg-gray-50 border-b border-gray-200">Stock Assets List</h3>
             ...
             <tbody>
               {rep.items.map((m, i) => (
                 <tr key={i} className={`border-b border-gray-50 hover:bg-gray-50/50 ${m.isLowStock ? 'bg-amber-50/30' : ''}`}>
                   ...
                   <td className="px-5 py-3 text-gray-600 truncate max-w-[200px]">{m.description} ({m.uom})</td>
                   <td className="px-5 py-3 text-xs text-gray-500">
                      <span className="px-2 py-1 bg-gray-100 rounded text-gray-600 font-bold">{m.category.replace('_',' ')}</span>
                   </td>
                   <td className="px-5 py-3 text-right tabular-nums">
                     <span className={m.isLowStock ? 'text-amber-700 font-bold' : ''}>{m.qty.toFixed(2)}</span>
                   </td>
                   <td className="px-5 py-3 text-right tabular-nums text-gray-500">{m.avgRate.toFixed(2)}</td>
                   <td className="px-5 py-3 text-right tabular-nums font-semibold">{formatCurrency(m.value)}</td>
                 </tr>
               ))}
             </tbody>
           </table>
