  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'Total WIP Orders',   value: Number(summary?.total ?? 0),                              color: 'text-gray-900'  },
          { label: 'Total WIP Value (₹)', value: formatCurrency(summary?.value ?? 0).replace('₹', ''),   color: 'text-rose-600'  },
        ].map((k, i) => (
            ...
            <tbody>
              {aging.map((a, i) => (
                <tr key={i} className="border-b border-gray-50">
                  ...
                  <td className="px-5 py-3 text-right tabular-nums font-bold">{Number(a?.count ?? 0)}</td>
                  <td className="px-5 py-3 text-right tabular-nums text-gray-600">{formatCurrency(a?.value ?? 0)}</td>
                </tr>
              ))}
              {aging.length === 0 && (
                <tr>
                  <td colSpan={3} className="px-5 py-8 text-center text-gray-400">No aging data.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {/* Detail Drilldown Table */}
      <div className="card p-0 overflow-hidden border border-gray-200">
        <h3 className="px-5 py-4 text-sm font-bold text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
          Open Work Orders Aging Drilldown
        </h3>
        <div className="overflow-x-auto max-h-[400px]">
          <table className="w-full text-sm">
            ...
            <tbody>
              {details.map((m, i) => {
                const ageDays = Number(m?.ageDays ?? 0);
                return (
                  <tr key={i} className={`border-b border-gray-50 hover:bg-gray-50/50 ${ageDays > 30 ? 'bg-red-50/30' : ''}`}>
                    ...
                    <td className={`px-5 py-3 text-right tabular-nums font-bold ${ageDays > 30 ? 'text-red-700' : 'text-gray-800'}`}>
                      {ageDays}
                    </td>
                    <td className="px-5 py-3 text-right tabular-nums font-semibold">
                      {Number(m?.wipQty   ?? 0).toFixed(2)}
                    </td>
                    <td className="px-5 py-3 text-right tabular-nums text-amber-700">
                      {formatCurrency(m?.wipValue ?? 0)}
                    </td>
                  </tr>
                );
              })}
              {details.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-5 py-8 text-center text-gray-400">No open work orders.</td>
                </tr>
              )}
            </tbody>
          </table>
