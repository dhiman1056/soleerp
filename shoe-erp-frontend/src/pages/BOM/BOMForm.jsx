...
const SIZE_PREVIEWS = {
  'INFANT': ['UK 2', 'UK 3', 'UK 5', 'UK 6', 'UK 7', 'UK 8', 'UK 9', 'UK 10', 'UK 11', 'UK 12'],
  'KIDS': ['UK 6', 'UK 7', 'UK 8', 'UK 9', 'UK 10', 'UK 11', 'UK 11.5', 'UK 12', 'UK 12.5', 'UK 13', 'UK 1', 'UK 2', 'UK 3', 'UK 4', 'UK 5', 'UK 6 (KIDS)'],
  'LADIES': ['UK 3', 'UK 4', 'UK 5', 'UK 6', 'UK 7', 'UK 8', 'UK 9'],
...
export default function BOMForm() {
  ...
  const [productSizeChart, setProductSizeChart] = useState(null)

  const activeSizes = useMemo(() => {
    if (!productSizeChart || !SIZE_PREVIEWS[productSizeChart]) return []
    return SIZE_PREVIEWS[productSizeChart].map(s => ({ size_code: s }))
  }, [productSizeChart])
  ...
  const { fields, append, remove, update: updateLine } = useFieldArray({ control, name: 'lines' })

  const watchedBomType = watch('bom_type')
  const watchedLines   = watch('lines')
  const watchedOutputQty = Number(watch('output_qty')) || 1
  const safeLines      = Array.isArray(watchedLines) ? watchedLines : []

  const outputProducts = useMemo(() => {
    return allProducts.filter((p) => {
      if (watchedBomType === 'SF') return p.product_type === 'SEMI_FINISHED'
      if (watchedBomType === 'FG') return p.product_type === 'FINISHED'
      return false
    })
  ...
  const handleOutputSkuChange = (sku_code) => {
    ...
    if (product) {
      setValue('output_uom', product.uom || 'PAIR')
      setValue('remarks', product.description || '')
      setProductSizeChart(product.size_chart || null)
    } else {
      setProductSizeChart(null)
    }
  }

  const handleComponentSkuChange = (idx, sku_code) => {
    const product = allProducts.find(p => p.sku_code === sku_code) || allRMs.find(r => r.sku_code === sku_code)
    if (product) {
      setValue(`lines.${idx}.description`, product.description || '')
      setValue(`lines.${idx}.uom`, product.uom || 'PCS')
      setValue(`lines.${idx}.rate_at_bom`, Number(product.rate || product.avg_rate || 0))
    }
  }
  ...
  useEffect(() => {
    ...
    // Trigger product lookup for size charts
    if (b.output_sku) handleOutputSkuChange(b.output_sku)
  }, [isEdit, bomData, reset, allProducts])
  ...
  const onSubmit = (values) => {
    ...
    if (useSizeVariants && activeSizes.length > 0) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-5xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
          <div>
            ...
            <input
              ...
              className="input-field uppercase bg-gray-50 text-gray-500 font-mono"
            />
          </div>

          <div>
            <label className="label">BOM Type *</label>
            <select {...register('bom_type', { required: true })} className="input-field" disabled={isEdit}>
          ...
          <div>
            ...
            <input
              ...
              {...register('output_qty', { required: true, min: 0.01, valueAsNumber: true })}
              className={`input-field ${errors.output_qty ? 'input-error' : ''}`}
            />
            {errors.output_qty && <p className="text-red-500 text-xs mt-1">Required</p>}
          </div>
          ...
          <div>
            <label className="label">Description (from Product)</label>
            <input 
              {...register('remarks')} 
              placeholder="Auto-filled from product description" 
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {fields.map((field, index) => {
                const consumeQty = Number(safeLines[index]?.consume_qty) || 0
                const rateAtBom = Number(safeLines[index]?.rate_at_bom) || 0
                const avgPerPair = consumeQty / watchedOutputQty
                const lineVal = consumeQty * rateAtBom
                ...
                return (
                  <tr key={field.id} className="border-b border-gray-50">
                    ...
                    <td className="px-3 py-2">
                      <input {...register(`lines.${index}.description`)} readOnly className="input-field text-xs bg-gray-50 text-gray-500 cursor-default" placeholder="Auto-filled" />
                    </td>
                    <td className="px-3 py-2">
                      <input type="number" step="0.0001" min="0.0001" {...register(`lines.${index}.consume_qty`, { required: true, min: 0.0001, valueAsNumber: true })} className="input-field text-xs text-right tabular-nums" />
                    </td>
                    <td className="px-3 py-2 text-right text-xs font-semibold tabular-nums text-blue-700 bg-blue-50/30">
                      {avgPerPair.toFixed(4)}
                    </td>
                    <td className="px-3 py-2">
                    ...
                    <td className="px-3 py-2">
                      <input type="number" step="0.01" min="0" {...register(`lines.${index}.rate_at_bom`, { valueAsNumber: true })} className="input-field text-xs text-right tabular-nums" />
                    </td>
                    ...
                    <td className="px-2 py-2">
                      {fields.length > 1 && (
                        <button type="button" onClick={() => remove(index)} className="p-1 rounded text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors" title="Remove row">✕</button>
                      )}
                    </td>
                  </tr>
                )
            ...
            <tfoot>
              <tr className="bg-gray-50 border-t-2 border-gray-200">
                <td colSpan={6} className="px-3 py-3 text-right text-sm font-bold text-gray-700">Total Material Cost</td>
      ...
      {productSizeChart && activeSizes.length > 0 && (
        <div className="card p-6 space-y-4 border border-blue-100 bg-blue-50/20">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Override material consumption for specific shoe sizes ({productSizeChart}).</p>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    {activeSizes.map(s => <th key={s.size_code} className="px-3 py-3 text-center text-xs font-bold text-blue-700 uppercase min-w-[90px]">{s.size_code}</th>)}
                  </tr>
                </thead>
                <tbody>
                      ...
                      {activeSizes.map(s => {
                        ...
                        return (
                          <td key={s.size_code} className="px-2 py-2">
                            <input type="number" step="0.0001" min="0.0001" placeholder={(Number(l.consume_qty) || 0).toFixed(3)} value={vQty !== undefined ? vQty : ''} onChange={(e) => handleVariantChange(s.size_code, l.input_sku, e.target.value)} className="input-field text-xs text-right tabular-nums py-1 px-2 w-full border-dashed focus:border-solid focus:border-blue-500" />
                          </td>
                        )
                      })}
                    </tr>
      ...
      <div className="flex items-center justify-end gap-3 border-t border-gray-200 pt-5 mt-5">
        <button type="button" onClick={() => navigate('/bom')} className="px-5 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 focus:outline-none disabled:opacity-50" disabled={isBusy}>Cancel</button>
        <button type="submit" className="px-5 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg shadow-sm hover:bg-blue-700 focus:outline-none disabled:opacity-50" disabled={isBusy}>
