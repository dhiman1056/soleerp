...
import { fetchBOMs }          from '../../api/bomApi.js'
import { formatCurrency }     from '../../utils/formatCurrency.js'
import { today }              from '../../utils/formatDate.js'
import { WO_TYPES, WO_TYPE_LABELS, WO_TO_BOM_TYPE } from '../../utils/constants.js'

const STORE_OPTIONS = ['Raw Material Store', 'WIP Store', 'Stitching & Cutting Floor', 'Assembly Floor', 'Finished Goods Warehouse']

export default function WorkOrderForm({ isOpen, onClose }) {
  const [selectedType, setSelectedType] = useState(null)

  const { register, handleSubmit, watch, setValue, reset, formState: { errors } } = useForm({
    defaultValues: { bom_id: '', wo_date: today(), planned_qty: 50, from_store: '', to_store: '', notes: '' },
  })

  const selectedBomId = watch('bom_id')
  const createMut     = useCreateWorkOrder()

  const { data: sizesRes } = useSizesQuery({ is_active: 'true' })
  ...
  // Fetch BOMs filtered by selected WO type
  const bomType = selectedType ? WO_TO_BOM_TYPE[selectedType] : null
  const { data: bomsData, isLoading: bomsLoading } = useQuery({
    queryKey: ['boms', { bom_type: bomType, is_active: 'true', limit: 200 }],
    queryFn:  () => fetchBOMs({ bom_type: bomType, is_active: 'true', limit: 200 }),
    enabled:  !!bomType,
  })
  const boms = bomsData?.data || []

  // Fetch selected BOM detail for preview
  const { data: selectedBomData } = useQuery({
    queryKey: ['boms', Number(selectedBomId)],
    queryFn:  () => import('../../api/bomApi.js').then(m => m.fetchBOM(selectedBomId)),
    enabled:  !!selectedBomId,
  })
  const selectedBom = selectedBomData?.data

  const { data: stockData } = useStockSummaryQuery()
  const stockSummary = stockData?.data || []

  // Check if any stock is insufficient against planned_qty
  const plannedQty = useSizeBreakup 
    ? Object.values(sizeBreakupMap).reduce((sum, val) => sum + (parseInt(val) || 0), 0)
    : (watch('planned_qty') || 0)

  // Use BOM's sizeVariants to calculate required stock exactly if size breakup is provided
  const isInsufficient = selectedBom?.lines?.some(l => {
    let required = 0
    if (useSizeBreakup && selectedBom.sizeVariants && Object.keys(sizeBreakupMap).length > 0) {
       Object.entries(sizeBreakupMap).forEach(([sCode, qty]) => {
          ...
          const unit = vQty !== undefined ? parseFloat(vQty) : parseFloat(l.consume_qty)
          required += unit * (parseInt(qty) || 0)
       })
    } else {
       required = parseFloat(l.consume_qty) * parseFloat(plannedQty)
    }
    const available = stockSummary.find(s => s.sku_code === l.input_sku)?.current_qty || 0
    return parseFloat(available) < (required || 0)
  })
  ...
  const onSubmit = (values) => {
    let finalBreakup = []
    if (useSizeBreakup) {
      Object.entries(sizeBreakupMap).forEach(([sizeCode, plannedQty]) => {
         if (parseInt(plannedQty) > 0) {
            finalBreakup.push({ sizeCode, plannedQty: parseInt(plannedQty) })
         }
      })
    }
        ...
        <form id="wo-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col gap-4">
               <div className="flex items-center justify-between">
                 ...
                 <div className="flex items-center gap-2 bg-white rounded-md p-1 shadow-sm border border-gray-200">
                    <button type="button" onClick={() => setUseSizeBreakup(false)} className={`px-3 py-1 text-xs font-semibold rounded transition-colors ${!useSizeBreakup ? 'bg-gray-800 text-white' : 'text-gray-600 hover:bg-gray-50'}`}>Total Qty</button>
                    <button type="button" onClick={() => setUseSizeBreakup(true)} className={`px-3 py-1 text-xs font-semibold rounded transition-colors ${useSizeBreakup ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'}`}>Size Breakup</button>
                 </div>
               </div>
               ...
               {!useSizeBreakup ? (
                 <div className="w-1/3">
                   ...
                   <input
                     ...
                     {...register('planned_qty', { required: !useSizeBreakup, min: { value: 1, message: 'Must be > 0' }, valueAsNumber: true })}
                 ...
                 <div className="overflow-x-auto bg-white border border-gray-200 rounded-lg shadow-sm">
                   <table className="w-full text-sm">
                     <thead className="bg-gray-50 border-b border-gray-200">
                       <tr>
                         {activeSizes.map(s => <th key={s.size_code} className="px-2 py-2 text-center text-xs text-gray-600 font-bold uppercase">{s.size_code}</th>)}
                     ...
                     <tbody>
                       <tr>
                         ...
                         <td className="px-3 py-2 text-right font-bold text-gray-800 bg-gray-50tabular-nums">{plannedQty}</td>
                       </tr>
                     </tbody>
                   </table>
                 </div>
            ...
            <div>
              ...
              <select {...register('from_store', { required: 'From store is required' })} className={`input-field ${errors.from_store ? 'input-error' : ''}`}>
            ...
            <div>
              ...
              <select {...register('to_store', { required: 'To store is required' })} className={`input-field ${errors.to_store ? 'input-error' : ''}`}>
                <option value="">— Select —</option>
                {STORE_OPTIONS.map((s) => <option key={s}>{s}</option>)}
              </select>
          ...
          {selectedBom?.lines?.length > 0 && (
            <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50/40 p-4">
                  ...
                  {selectedBom.lines.map((l, i) => {
                    ...
                    if (useSizeBreakup && selectedBom.sizeVariants && Object.keys(sizeBreakupMap).length > 0) {
                      Object.entries(sizeBreakupMap).forEach(([sCode, qty]) => {
                       ...
                       required = parseFloat(l.consume_qty) * plannedQty
                    }
                    const requiredFmt = required.toFixed(3)
                    const available = parseFloat(stockSummary.find(s => s.sku_code === l.input_sku)?.current_qty || 0).toFixed(3)
                    const isOk = parseFloat(available) >= parseFloat(requiredFmt)
                    return (
                      <tr key={i} className="border-t border-blue-100">
                        ...
                        <td className="py-2 text-right tabular-nums">{required}</td>
                        <td className="py-2 text-right tabular-nums">{available}</td>
              ...
              <div className="mt-2 pt-2 border-t border-blue-100 flex justify-end">
                <span className="text-xs font-bold text-blue-900">Est. Total Cost: {formatCurrency(selectedBom.total_material_cost * plannedQty)}</span>
              </div>
            </div>
          )}
        </form>
