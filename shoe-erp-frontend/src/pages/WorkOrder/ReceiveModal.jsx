...
export default function ReceiveModal({ isOpen, onClose, wo }) {
  ...
  useEffect(() => {
    if (isOpen) {
       ...
       if (hasSizes) {
         const initialRe = {}
         wo.sizeBreakup.forEach(s => initialRe[s.size_code] = '')
         setReceiptsMap(initialRe)
       } else {
         setReceiptsMap({})
       }
    }
  ...
  return (
    ...
    >
      ...
      <form id="receive-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {!hasSizes ? (
          <div>
            <label className="label">Received Qty * (max {wipQty.toFixed(2)})</label>
            <input
              type="number"
              step="0.01"
              min="0.01"
          ...
          <div>
            <label className="label mb-2">Receive Quantities by Size</label>
               ...
               <table className="w-full">
                 ...
                 <tfoot className="bg-gray-50 border-t border-gray-200">
                    <tr>
                       ...
                       <td className="px-3 py-2 text-center text-blue-700 font-bold tabular-nums text-base">
                         {Object.values(receiptsMap).reduce((s, v) => s + (parseInt(v)||0), 0)}
                       </td>
                    </tr>
                 </tfoot>
               </table>
