...
export default function UserManagement() {
  const { user } = useAuth();
  // queryFn now returns a plain array (never undefined or Axios wrapper)
  ...
  // data is already the array; guard just in case
...
function UserModal({ userObj, onClose }) {
  ...
  const [form, setForm] = useState(
    userObj ?? { name: '', email: '', password: '', role: 'operator', phone: '', department: '', is_active: true }
  );
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (isEdit) {
      updateMut.mutate({ id: userObj.id, ...form }, {
        onSuccess: () => { toast.success('User updated.'); onClose(); },
      });
    } else {
      createMut.mutate(form, {
        onSuccess: () => { toast.success('User provisioned.'); onClose(); },
      });
    }
  };
  ...
  const doReset = () => {
    ...
    resetMut.mutate({ id: userObj.id, new_password: resetPwd }, {
      onSuccess: () => { toast.success('Password reset successfully.'); setResetPwd(''); },
    });
  };
    ...
    <div className="fixed inset-0 bg-black/50 overflow-y-auto w-full h-full flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-xl p-6 relative">
        <h2 className="text-xl font-bold text-gray-900 mb-6">
          {isEdit ? 'Manage User' : 'Provision User'}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required className="input-field mt-1" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-600">Email Address *</label>
              <input required type="email" className="input-field mt-1" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} />
            </div>

            {!isEdit && (
              <div className="col-span-2">
                <label className="text-xs font-semibold text-gray-600">Initial Password *</label>
                <input required type="text" className="input-field mt-1" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} />
              </div>
            )}
            ...
            <div>
              ...
              <input className="input-field mt-1" value={form.department} onChange={e => setForm(f => ({ ...f, department: e.target.value }))} />
            </div>
            <div className="col-span-2">
              <label className="text-xs font-semibold text-gray-600">Phone</label>
              <input className="input-field mt-1" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
            </div>
          ...
          <div className="flex justify-end gap-3 pt-6 mt-6 border-t border-gray-100">
            ...
            <button
              ...
              className="btn-primary"
            >
              {createMut.isPending || updateMut.isPending ? 'Saving…' : 'Save Configuration'}
            </button>
          </div>
        </form>
          ...
          <div className="mt-8 p-4 border border-red-200 bg-red-50 rounded-lg">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="input-field py-1 text-sm bg-white border-red-300"
              />
              <button
                type="button"
                onClick={doReset}
                disabled={resetMut.isPending}
                className="btn-secondary border-red-300 text-red-700 hover:bg-red-100 py-1 font-bold shadow-none"
              >
                Execute
              </button>
            </div>
