...
const createRules = [
  body('bom_id')
    .notEmpty().withMessage('bom_id is required')
    .isInt({ gt: 0 }).withMessage('bom_id must be a positive integer'),
  body('planned_qty')
    .notEmpty().withMessage('planned_qty is required')
  ...
  body('from_store')
    .trim().notEmpty().withMessage('from_store is required')
    .isLength({ max: 100 }),
  body('to_store')
    .trim().notEmpty().withMessage('to_store is required')
    .isLength({ max: 100 }),
  body('wo_date')
    .optional().isDate().withMessage('wo_date must be a valid date (YYYY-MM-DD)'),
  body('notes')
    .optional(),
];
